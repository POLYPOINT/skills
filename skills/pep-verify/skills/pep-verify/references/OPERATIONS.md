...
All three landed on `pep-driver` `main` in the 2026-07-27 merge. On an older checkout only SSH works
— if `grep -l ansible_connection cli/inventory.py` comes up empty, pull. `preflight.sh` warns when
the checkout is behind origin or carries uncommitted local-only changes.
