...
| `TDateTimePicker` / `TPEPDateEdit` / `TDBDateEdit`                              | `<pp-datepicker label="Date" [formControl]="control" />`                                                                                   | Use `@pdx/pp-datepicker` `PPDatepickerComponent`. Implements `ControlValueAccessor`. Commits a `Date`; convert to `Temporal.PlainDate` at the store/service boundary. For a start/end pair use `type="range"` and bind to `PPDateRange`.                                                                 |
...
| Delphi VCL                          | Angular                                                                                                | Notes                                                                                                                                                                                                  |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `TDBGrid`                           | `<table mat-table>` with `matSort`                                                                     | Column defs per field. For paged results, pair with `<pp-paginator>` (next row).                                                                                                                       |
| Paged `TDBGrid` / paged list panels | `<pp-paginator [length]="total()" [(pageIndex)]="page" [(pageSize)]="size" (page)="onPage($event)" />` | Use `@pdx/pp-paginator` `PPPaginatorComponent`. `pageIndex` is zero-based (Material convention); page-size options hard-coded to `[10, 25, 50, 100]`. Pair with `mat-table`. Replaces `mat-paginator`. |
